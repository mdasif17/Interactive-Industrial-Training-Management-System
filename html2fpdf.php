...
//! @return An object (a class instance)
        ...
        $this->lineheight = 1; // Related to FontSizePt == 11
        ...
        foreach ($a as $i => $e) {
            ...
            if ($i % 2 == 0) {
                ...
                if (strpos($e, "&") !== false) { //HTML-ENTITIES decoding
                    if (strpos($e, "#") !== false)
                        $e = value_entity_decode($e); // Decode value entities
                        
//Avoid crashing the script on PHP 4.0
                    $version = phpversion();
                    $version = str_replace('.', '', $version);
                    if ($version >= 430)
                        $e = html_entity_decode($e, ENT_QUOTES, 'cp1252'); // changes &nbsp; and the like by their respective char
                    else
        ...
        switch ($tag) {
            ...
            case 'OUTLINE': //custom-tag (CSS2 property - browsers don't support it yet - Jan2005)
                ...
                if (isset($attr['COLOR']))
                    ...
                    $this->outlineparam['COLOR'] = array('R' => 255, 'G' => 255, 'B' => 255); //white
                $this->outlineparam['OLDWIDTH'] = $this->LineWidth;
                if (isset($attr['WIDTH'])) {
                    switch (strtoupper($attr['WIDTH'])) {
                        case 'THIN': $this->outlineparam['WIDTH'] = 0.75 * $this->LineWidth;
                            break;
                        case 'MEDIUM': $this->outlineparam['WIDTH'] = $this->LineWidth;
                            break;
                        case 'THICK': $this->outlineparam['WIDTH'] = 1.75 * $this->LineWidth;
                            break;
                    }
                } else
            ...
            case 'OL':
                if (!isset($attr['TYPE']) or $attr['TYPE'] == '')
                    $this->listtype = '1'; //OL default == '1'
                else
                    $this->listtype = $attr['TYPE']; // ol and ul types are mixed here
            case 'UL':
            ...
            case 'U':
                if (isset($attr['CLASS']) or isset($attr['ID']) or isset($attr['STYLE'])) {
                    ...
                    if (!empty($properties))
                        $this->setCSS($properties); //name found in the CSS array!
                }
            ...
            case 'IMG':
                ...
                if (isset($attr['SRC'])) {
                    ...
                    if (!isset($attr['WIDTH']))
                        ...
                        $attr['WIDTH'] = ConvertSize($attr['WIDTH'], $this->pgwidth); //$attr['WIDTH'] /= 4;
                    if (!isset($attr['HEIGHT']))
                        $attr['HEIGHT'] = 0;
                    else
                        $attr['HEIGHT'] = ConvertSize($attr['HEIGHT'], $this->pgwidth); //$attr['HEIGHT'] /= 4;
                    if ($this->tdbegin) {
                        $bak_x = $this->x;
                        $bak_y = $this->y;
            ...
            case 'BR':
                if ($this->tablestart) {
                    ...
                    if (!isset($this->cell[$this->row][$this->col]['maxs']))
                        $this->cell[$this->row][$this->col]['maxs'] = $this->cell[$this->row][$this->col]['s'] + 2; //+2 == margin
            ...
            case 'P':
                ...
                if (isset($attr['CLASS']) or isset($attr['ID']) or isset($attr['STYLE'])) {
                    ...
                    $this->backupcss = $properties;
                    if (!empty($properties))
                        $this->setCSS($properties); //name(id/class/style) found in the CSS array!
                }
                break;
            case 'SPAN':
                $this->buffer_on = true;
            ...
            case 'FORM':
                if ($this->tablestart) {
                    ...
                    $this->Ln($this->lineheight); //Skip a line, if needed
                break;
            case 'INPUT':
                if (!isset($attr['TYPE']))
                    $attr['TYPE'] == ''; //in order to allow default 'TEXT' form (in case of malformed HTML code)
                if (!$this->tablestart) {
...
//! @return void
            ...
            if (!$this->pbegin and ! $this->divbegin and ! $this->tablestart) {
                if ($this->cssbegin) {
                    ...
                    if ($this->cssbegin and ( $this->divborder or $this->dash_on or $this->dotted_on or $this->divbgcolor)) {
                        ...
                        $tempx = $this->x;
                        if ($this->divbgcolor)
                            $this->Cell($this->GetStringWidth($texto), $this->lineheight, '', $this->divborder, '', 'L', $this->divbgcolor);
                        if ($this->dash_on)
                            $this->Rect($this->oldx, $this->oldy, $this->GetStringWidth($texto), $this->lineheight);
                        if ($this->dotted_on)
                            $this->DottedRect($this->x - $this->GetStringWidth($texto), $this->y, $this->GetStringWidth($texto), $this->lineheight);
                        $this->x = $tempx;
        ...
        if (($tag == 'UL') or ( $tag == 'OL')) {
            if ($this->buffer_on == false)
                $this->listnum--; //Adjust minor BUG (this happens when there are two </OL> together)
        ...
        if ($tag == 'PRE') {
            ...
            if ($this->divbegin or $this->pbegin or $this->buffer_on)
                $this->textbuffer[] = array("\n", $this->HREF, $this->currentstyle, $this->colorarray, $this->currentfont, $this->SUP, $this->SUB, ''/* internal link */, $this->strike, $this->outlineparam, $this->bgcolorarray);
            $this->cssbegin = false;
            $this->backupcss = array();
            $this->buffer_on = false;
        ...
        if ($tag == 'TEXTAREA') {
            if (!$this->tablestart) { //not inside a table
                ...
                if ($texto != '')
                    $this->MultiCell((2.2 * $this->col) + 6, $this->lineheight, $texto);
        ...
        if ($tag == 'SELECT') {
            ...
            if (isset($this->selectoption['MAXWIDTH']))
                $tamanho = $this->selectoption['MAXWIDTH'];
            if ($this->tablestart) {
...
//! @desc Prepares buffered text to be printed with FlowingBlock()
        ...
        if ($is_table) {
            ...
            $this->divheight = $this->lineheight;
            if ($this->FillColor == '1.000 g')
                $fill = 0; //avoid useless background painting (1.000 g == white background color)
            else
                $fill = 1;
        }
        ...
        for ($i = 0; $i < $array_size; $i++) {
            ...
            if (empty($vetor[0]) and empty($vetor[7]))
                continue; //Ignore empty text and not carrying an internal link
...
//Activating buffer properties
            ...
            if (isset($vetor[4]) and $vetor[4] != '')
                $this->SetFont($vetor[4]); // Font Family
            if (!empty($vetor[3])) { //Font Color
                $cor = $vetor[3];
                $this->SetTextColor($cor['R'], $cor['G'], $cor['B']);
            }
            if (isset($vetor[2]) and $vetor[2] != '') { //Bold,Italic,Underline styles
                if (strpos($vetor[2], "B") !== false)
                    $this->SetStyle('B', true);
                if (strpos($vetor[2], "I") !== false)
                    $this->SetStyle('I', true);
                if (strpos($vetor[2], "U") !== false)
                    $this->SetStyle('U', true);
            }
            ...
            if (isset($vetor[0]) and $vetor[0]{0} == '�' and $vetor[0]{1} == '�' and $vetor[0]{2} == '�') { //identifier has been identified!
                ...
                if ($this->flowingBlockAttr['contentWidth'] > 0) { // Print out previously accumulated content
                    ...
                    if ($specialcontent['type'] == 'select')
                        $extrawidth = 7; //arrow box + margin
                ...
                switch (strtoupper($specialcontent['type'])) {
                    case 'IMAGE':
                        ...
                        $xpos = 0;
                        $ypos = 0;
                        if (isset($specialcontent['ypos']) and $specialcontent['ypos'] != '')
                            $ypos = (float) $specialcontent['ypos'];
                        if (isset($specialcontent['xpos']) and $specialcontent['xpos'] != '')
                            $xpos = (float) $specialcontent['xpos'];
                    ...
                    case 'INPUT':
                        switch ($specialcontent['subtype']) {
                            ...
                            case 'RADIO': //Draw Radio button
                                ...
                                $this->_out('0.000 g');
                                if ($checked)
                                    $this->Circle($this->x, $this->y + 1.2, 0.4, 'DF');
                                $this->y -= 1.5;
                                //Readjust x coordinate in order to allow text to be placed after this form element
                                $this->x = $bak_x;
                                $spacesize = $this->CurrentFont['cw'][' '] * ( $this->FontSizePt / 1000 );
                    ...
                    case 'SELECT':
                        ...
                        $this->x += 1;
                        if ($texto != '')
                            $this->Write($this->lineheight, $texto, $this->x); //the combobox content
                    ...
                    case 'TEXTAREA':
                        ...
                        if ($texto != '')
                            $this->MultiCell($ta_width + 3, $this->lineheight, $texto);
                        $this->y = $backup_y - 1.5;
                ...
                if ($vetor[0] == "\n") { //We are reading a <BR> now turned into newline ("\n")
                    ...
                    $this->WriteFlowingBlock($vetor[0], $outofblock);
            }
        ...
        if ($this->cssbegin and ( $this->divborder or $this->dash_on or $this->dotted_on or $this->divbgcolor)) {
            ...
            if ($this->divborder)
                $this->Rect($x, $y, $this->divwidth, $old_height);
            if ($this->dash_on)
                $this->Rect($x, $y, $this->divwidth, $old_height);
            if ($this->dotted_on)
                $this->DottedRect($x, $y, $this->divwidth, $old_height);
            $this->x = $bak_x;
        }
    }
        ...
        foreach ($arrayaux as $k => $v) {
            switch ($k) {
                ...
                case 'BORDER': // width style color (width not supported correctly - it is always considered as normal)
                    $prop = explode(' ', $v);
                    if (count($prop) != 3)
                        break; // Not supported: borders not fully declared
        ...
        for ($j = 0; $j < $nc; $j++) { //columns
            ...
            for ($i = 0; $i < $nr; $i++) { //rows
                if (isset($cs[$i][$j]) && $cs[$i][$j]) {
                    ...
                    if ($wc['miw'] > $wc['maw'])
                        $wc['maw'] = $wc['miw']; //update maximum width, if needed
                }
            }//rows
        }//columns
            ...
            for ($j = 0; $j < $numcols; $j++) { //columns
                if (isset($cells[$i][$j]) && $cells[$i][$j]) {
                    ...
                    if ($c['s'] > $nostyles_size and ! isset($c['form'])) { //Text with styles
                        ...
                        if ($linesneeded == 0)
                            $linesneeded = 1; //to avoid division by zero
                    ...
                    if (isset($c['form'])) {
                        ...
                        if (($this->GetStringWidth($auxtext) + 3) > ($cw - 2))
                            $linesneeded++;
                    }
        ...
        foreach ($listspan as $span) {
            ...
            if ($c['mih'] > $hs) {
                if (!$hs) {//Cac dong chua co kich thuoc => chia deu
                    ...
                    $hi = $c['mih'] - $hsa;
                    foreach ($list as $k)
                        $heightrow[$k] += ($heightrow[$k] / $hsa) * $hi;
                }
            }
        }
    }
            ...
            for ($j = 0; $j < $numcols; $j++) { //Columns
                if (isset($cells[$i][$j]) && $cells[$i][$j]) {
                    ...
                    if (!empty($cell['textbuffer']))
                        $this->printbuffer($cell['textbuffer'], false, true/* inside a table */);
                    //Reset values
                    $this->Reset();
                }//end of (if isset(cells)...)
            }// end of columns
            if ($i == $numrows - 1)
                $this->y = $y + $h; //last row jump (update this->y position)
        }// end of rows
    }
